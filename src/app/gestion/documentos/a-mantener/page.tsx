...
const DocumentosAMantenerPage = () => {
  const [documentName, setDocumentName] = useState('');
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [selectedProcess, setSelectedProcess] = useState<string>('');
  const [documentType, setDocumentType] = useState<'mantener' | 'conservar' | 'formato'>('mantener');
  ...
  const handleUpload = () => {
    if (documentName && selectedFile) {
      ...
      setSelectedFile(null);
      setSelectedProcess('');
      setDocumentType('mantener'); // Reset to default
  ...
  return (
    <div className="module-container">
      <h1 className="module-title">Documentos a Mantener</h1>
      <p className="module-description">Gestione los documentos que deben ser mantenidos.</p>
      ...
      <div className="form-section mt-8">
        <h2 className="form-title">Documentos Subidos</h2>
        {documents.length === 0 ? (
...
export default DocumentosAMantenerPage;
