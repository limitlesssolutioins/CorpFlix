...
async function initHrDatabaseCorrected() {
    ...
    console.log(`🚀 CORRIGIENDO Y COMPLETANDO BASE DE DATOS: ${companyId}`);

    const db = new sqlite3.Database(dbPath);
    const run = (sql) => new Promise((res, rej) => db.run(sql, (err) => err ? rej(err) : res()));

    try {
        // Tablas ya creadas (usamos IF NOT EXISTS)
        ...
        await run(`CREATE TABLE IF NOT EXISTS Employee (id TEXT PRIMARY KEY, firstName TEXT NOT NULL, lastName TEXT NOT NULL, identification TEXT UNIQUE NOT NULL, email TEXT, phone TEXT, contractType TEXT NOT NULL, salaryAmount REAL DEFAULT 0, salaryScheme TEXT DEFAULT 'FIJO', startDate DATE, epsEntity TEXT, arlEntity TEXT, pensionEntity TEXT, defaultSiteId TEXT, defaultPositionId TEXT, teamId TEXT, standardStartTime TEXT, standardEndTime TEXT, standardStartTime2 TEXT, standardEndTime2 TEXT, workDays TEXT, isActive INTEGER DEFAULT 1, createdAt DATETIME DEFAULT CURRENT_TIMESTAMP, updatedAt DATETIME DEFAULT CURRENT_TIMESTAMP)`);

        // CORRECCIÓN: Tabla de Ausencias (debe llamarse Absence)
        ...
        console.log('✅ Base de datos verificada y corregida.');
    } catch (e) {
